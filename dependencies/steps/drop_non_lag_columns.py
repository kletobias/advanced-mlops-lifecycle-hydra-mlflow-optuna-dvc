# dependencies/steps/drop_non_lag_columns.py
import logging
...
@log_function_call
def step_drop_non_lag_columns(cfg: RootConfig) -> None:
    cfg_step = DropNonLagColumnsConfig(**cfg.transformations.drop_non_lag_columns)
    df: pd.DataFrame = csv_to_dataframe(**cfg.utility_functions.csv_to_dataframe)
    df = drop_non_lag_columns(df, **cfg_step.__dict__)
