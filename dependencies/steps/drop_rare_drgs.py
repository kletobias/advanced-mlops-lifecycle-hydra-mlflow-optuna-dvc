# dependencies/steps/drop_rare_drgs.py
import logging
...
@log_function_call
def step_drop_rare_drgs(cfg: RootConfig) -> None:
    cfg_step = DropRareDrgsConfig(
        **cfg.transformations.drop_rare_drgs.options_drop_rare_drgs,
    )
    df: pd.DataFrame = csv_to_dataframe(**cfg.utility_functions.csv_to_dataframe)
    df = drop_rare_drgs(df, **cfg_step.__dict__)
