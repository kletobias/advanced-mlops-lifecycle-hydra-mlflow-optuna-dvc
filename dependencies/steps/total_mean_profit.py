# dependencies/steps/total_mean_profit.py
import logging
...
@log_function_call
def step_total_mean_profit(cfg: RootConfig) -> None:
    cfg_step = TotalMeanProfitConfig(**cfg.transformations.total_mean_profit)
    df: pd.DataFrame = csv_to_dataframe(**cfg.utility_functions.csv_to_dataframe)
    df = total_mean_profit(df, **cfg_step.__dict__)
