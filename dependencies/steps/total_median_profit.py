# dependencies/steps/total_median_profit.py
import logging
...
@log_function_call
def step_total_median_profit(cfg: RootConfig) -> None:
    cfg_step = TotalMedianProfitConfig(
        **cfg.transformations.total_median_profit.options_total_median_profit,
    )
    df: pd.DataFrame = csv_to_dataframe(**cfg.utility_functions.csv_to_dataframe)
    df = total_median_profit(df, **cfg_step.__dict__)
