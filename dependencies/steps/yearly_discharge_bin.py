...
@log_function_call
def step_yearly_discharge_bin(cfg: RootConfig) -> None:
    cfg_step = YearlyDischargeBinConfig(**cfg.transformations.yearly_discharge_bin)
    df: pd.DataFrame = csv_to_dataframe(**cfg.utility_functions.csv_to_dataframe)
    df = yearly_discharge_bin(df, **cfg_step.__dict__)
