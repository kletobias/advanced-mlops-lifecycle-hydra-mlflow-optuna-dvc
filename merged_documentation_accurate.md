...
Switching data versions or transformations can be done with a few flags at runtime, rather than rewriting script after script. For example, if you want to run the **Optuna** trial used in our pipeline, match the exact line from the DVC stage `v13_rf_optuna_trial`:
...
By default, config.yaml sets a baseline for each group (e.g., data_versions=base, models=rf_optuna_trial_params). With Hydra, you can override them at runtime. For example, if you want to replicate the stage v6_drop_rare_drgs:
...
You can stack multiple overrides if your stage includes additional arguments, or if you want a custom run combining multiple overrides. Hydra merges them in memory, so the pipeline uses exactly what you specify at runtime—no duplication.
...
Suppose you want to run a transformation from v1 to v2 as in v1_drop_description_columns:
...
For certain stages—like the rf_optuna_trial—we set io_policy.WRITE_OUTPUT=false to avoid saving a new CSV:
...
This stops the pipeline from creating extra output files but still logs metrics and artifacts to MLflow.

	•	Model Tuning with Optuna
As above, the rf_optuna_trial or ridge_optuna_trial can be invoked with parameters from the DVC stage specifying the script base name, transformations, and the data version input. Just run the command that matches your pipeline stage in dvc.yaml, and Hydra takes care of the rest.
...
- name: v10_lag_columns
  ...
  overrides: setup.script_base_name=lag_columns transformations=lag_columns data_versions.data_version_input=v10 data_versions.data_version_output=v11
...

DVC uses these definitions to know which outputs (e.g., v11.csv) must be tracked.
...
shift_periods: 1
	...
	2.	Runtime Command
Per the v10_lag_columns stage in DVC, we use:
